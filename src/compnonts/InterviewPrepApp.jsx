...
const topics = [
    ...
    { id: 'javascript', name: 'JavaScript', dataSource: 'javascriptQuestionData' },
        ...
        <>
            {text.split('\n').map((line, index) => (
                <React.Fragment key={index}>
                    {line.trim() && (
                        <p className={`mb-3 ${line.endsWith(':') ? 'font-medium mt-4 text-cyan-300' : 'text-cyan-200'}`}>
                            {line}
                        </p>
                    )}
                </React.Fragment>
...
const InterviewPrepApp = ({ dataSources }) => {
    ...
    const getActiveData = () => {
        const activeSource = topics.find(topic => topic.id === activeTab)?.dataSource;
    ...
    const availableTopics = topics.filter(topic =>
        dataSources[topic.dataSource] && dataSources[topic.dataSource].length > 0
    );
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black text-white font-serif">
            <div className="bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-700 text-white py-16">
                <div className="container mx-auto px-4">
                    <h1 className="text-5xl font-bold mb-4 font-sans animate-fade-in">Interview Preparation Guide</h1>
                    <p className="text-xl font-light tracking-wide">Master your technical interviews with comprehensive answers and examples</p>
                </div>
            </div>
                    ...
                    <div className="flex space-x-4 overflow-x-auto">
                        {availableTopics.map(topic => (
                            <button
                                ...
                                className={`py-4 px-6 font-medium transition-all duration-300 relative ${activeTab === topic.id
                                    ? 'text-cyan-400'
                        ...
                        >
                            <div className="p-6">
                                <div className="flex flex-col space-y-4">
                                    <div className="flex justify-between items-start group" onClick={() => handleQuestionClick(question.id)}>
                                        <h3 className="text-lg font-medium text-cyan-300 flex-1 group-hover:text-cyan-400 transition-colors">
                                            <span className="font-semibold text-lg text-cyan-500">{index + 1}:</span> {question.question}
                                        </h3>

                                        <button
                                            className="ml-4 p-2 hover:bg-cyan-900/30 rounded-full transition-colors"
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {question.actionWords.map((word, index) =>
                                                word.trim() !== "" ? (
                                                    <span
                                                        key={index}
                                                        className="bg-gradient-to-r from-cyan-900/50 to-indigo-900/50 text-cyan-300 px-4 py-1 rounded-full text-sm font-medium shadow-lg border border-cyan-800/30 line-clamp-2"
                                                    >
                                                        {word.trim()}
                                                    </span>
                                                ) : null
                                ...
                                {expandedQuestion === question.id && (
                                    <div className="mt-6 pt-6 border-t border-gray-700">
                                        <div className="mb-6">
                                            <h4 className="text-lg font-medium text-cyan-300 mb-4">Answer</h4>
                                        ...
                                        {question.codeExample && (
                                            <div>
                                                <h4 className="text-lg font-medium text-cyan-300 mb-3">Code Example</h4>
                                                <pre className="bg-gray-900 text-cyan-200 p-6 rounded-xl overflow-x-auto shadow-lg border border-gray-700">
