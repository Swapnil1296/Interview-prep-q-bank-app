...
const topics = [
    ...
    { id: 'rrd', name: 'Router-Dom', dataSource: 'reactRouterDom' },
    { id: 'typescript', name: 'Typescript', dataSource: 'typeScript' },
    { id: 'git&github', name: 'Git/Github', dataSource: 'gitGithub' },
        ...
        <>
            {text?.split('\n').map((line, index) => (
                <React.Fragment key={index}>
                    {line?.trim() && (
                        <p className={`mb-3 ${line?.endsWith(':') ? 'font-medium mt-4 text-cyan-500' : 'text-cyan-200'}`}>
                            {line}
                        </p>
                    )}
                </React.Fragment>
...
const InterviewPrepApp = ({ dataSources }) => {
    ...
    const availableTopics = topics.filter(topic =>
        dataSources?.[topic.dataSource]?.length > 0
    );
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    {availableTags.map(tag => (
                        <button
                            ...
                            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 flex items-center gap-2 capitalize
                                ${selectedTag === tag
                            ...
                            <div className="p-6">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {question?.tags?.map((tag, index) => (
                                            <span
                                                ...
                                                className={`px-3 py-1 rounded-full text-sm cursor-pointer transition-colors ${selectedTag === tag
                                ...
                                {expandedQuestion === question?.id && (
                                    <div className="mt-6 pt-6 border-t border-gray-700">
                                        <div className="mb-6">
                                            <h4 className="text-lg font-medium text-cyan-300 mb-4 underline">Answer</h4>
